...
(function () {
  ...
  function errorView(route) {
    return '' +
      '<section class="route-error container">' +
        '<p class="eyebrow eyebrow--flame">Something went wrong</p>' +
  ...
  function notFound() {
    return '' +
      '<section class="route-error container">' +
        '<p class="eyebrow eyebrow--flame">404</p>' +
        '<h1 class="route-error__title">This skewer slipped off the grill.</h1>' +
  ...
  function finish(p) {
    ...
    document.title = (ROUTES[p] && ROUTES[p].title) || "Saté Ria";
    ...
    finish._ready = true;
    window.scrollTo({ top: 0, behavior: finish._ready ? "auto" : "auto" });
  }
